...
(async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({status:'passed', visibleTextChars:body.length, uiCrud, path:url.pathname}));
  } finally {
    if (browser) await browser.close();
  }
})().catch(error => {console.log(JSON.stringify({status:'failed', message:error.message}));process.exitCode=1;});
